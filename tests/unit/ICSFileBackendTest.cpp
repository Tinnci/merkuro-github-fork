...
protected:
    ...
    void TearDown() override { tempFile.remove(); }
...
TEST_F(ICSFileBackendTest, MultipleEvents)
{
    ...
    for (int i = 0; i < 10; ++i) {
        auto event =
            createTestEvent(QLatin1String("event-") + QString::number(i), QLatin1String("Event ") + QString::number(i));
        EXPECT_TRUE(backend.createEvent(event));
    }

    // Retrieve and verify
    for (int i = 0; i < 10; ++i) {
        auto event = backend.getEvent(QLatin1String("event-") + QString::number(i));
