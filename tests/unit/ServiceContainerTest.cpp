...
public:
    ...
    QList<CalendarEventPtr> getEventsByDateRange(
        const QDate& start, const QDate& end) override
    {
        QList<CalendarEventPtr> result;
        for (const auto& event : m_events) {
            if (event->startDateTime.date() >= start && 
                event->endDateTime.date() <= end) {
                result.append(event);
            }
        }
        return result;
    ...
    QList<CalendarEventPtr> getEventsByCollection(
    ...
    {
        return m_calendarNames.value(id, QLatin1String(""));
    }
...
TEST_F(ServiceContainerTest, CreateEventThroughService)
{
    ...
    ops->createEvent(event,
        [&created](const CalendarEventPtr&) { created = true; },
        [](const QString&) { }
    );
