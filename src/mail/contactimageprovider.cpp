...
{
    return new ThumbnailResponse(email, requestedSize, &m_pool);
}

ThumbnailResponse::ThumbnailResponse(QString email, QSize size, QThreadPool *pool)
...
{
    qDebug() << "Requesting image for" << m_email;
    QImage cachedImage;
        ...
        if (job->error() == KJob::NoError) {
            ...
            if (contactSize >= 1) {
                ...
                if (searchPhoto(searchJob->contacts())) {
                    qDebug() << "Found image" << m_photo.isIntern();
