 ...
 */
class DailyEventsModel : public QAbstractItemModel
{
    Q_OBJECT
    Q_PROPERTY(EventOccurrenceModel* model WRITE setModel)
public:
    DailyEventsModel(QObject *parent = nullptr);
    ~DailyEventsModel() = default;
...
private:
    ...
    QVariantList eventsOnDay(const QDate &rowStart) const;
