...
Kirigami.Page {
    ...
    function moveToSelected() {
        if (selectedDate.getDate() > 1) {
            pathView.currentItem.scheduleListView.positionViewAtIndex(selectedDate.getDate() - 1, ListView.Beginning);
        } else {
            pathView.currentItem.scheduleListView.positionViewAtBeginning()
        }
    }
    ...
    actions {
        left: Kirigami.Action {
            ...
            onTriggered: pathView.decrementCurrentIndex()//setToDate(DateUtils.previousMonth(startDate))
            displayHint: Kirigami.DisplayHint.IconOnly
        }
        right: Kirigami.Action {
            icon.name: "go-next"
            text: i18n("Next month")
            onTriggered: pathView.incrementCurrentIndex()//setToDate(DateUtils.nextMonth(startDate))
            displayHint: Kirigami.DisplayHint.IconOnly
        }
