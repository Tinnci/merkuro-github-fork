...
void TodoSortFilterProxyModel::emitDateDataChanged(const QModelIndex &idx)
{
    ...
    // Change the data we are using for the section headers in the view
    const auto bottomRow = idxRowCount - 1;
    const auto bottomRightIdx = index(bottomRow, DueDateColumn, idx);
    const auto topLeftIdx = index(0, DueDateColumn, idx);
    Q_EMIT dataChanged(topLeftIdx, bottomRightIdx, {DisplayDueDateRole, TopMostParentDueDateRole});

    const auto recurseOverChildren = [this, &idx, &srcModel](const int row) {
