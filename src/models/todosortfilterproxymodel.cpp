...
void TodoSortFilterProxyModel::emitDateDataChanged(const QModelIndex &idx)
{
    ...
    const auto iterateOverChildren = [this, &idx, &srcModel, &currentDate](const int row) {
        const auto childIdx = index(row, 0, idx);
        const auto isOverdue = childIdx.data(IsOverdueRole).toBool();

        if (isOverdue || m_lastDateRefreshDate != currentDate) {
