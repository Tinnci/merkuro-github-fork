...
TreeListView {
    ...
    property int showCompleted: Merkuro.TodoSortFilterProxyModel.ShowAll
    property int sortBy: Merkuro.TodoSortFilterProxyModel.SummaryColumn
    ...
    section {
        criteria: sortBy === Merkuro.TodoSortFilterProxyModel.SummaryColumn ?
            ViewSection.FirstCharacter : ViewSection.FullString
        property: switch(sortBy) {
            case Merkuro.TodoSortFilterProxyModel.PriorityColumn:
                return "topMostParentPriority";
            case Merkuro.TodoSortFilterProxyModel.DueDateColumn:
                return "topMostParentDueDate";
            case Merkuro.TodoSortFilterProxyModel.SummaryColumn:
            default:
        ...
        delegate: Kirigami.ListSectionHeader {
            ...
            readonly property bool dateSort: root.sortBy === Merkuro.TodoSortFilterProxyModel.DueDateColumn
    ...
    Kirigami.PlaceholderMessage {
        ...
        visible: (!Merkuro.Filter.collectionId || Merkuro.Filter.collectionId < 0) && Merkuro.CalendarManager.enabledTodoCollections.length === 0 && parent.count === 0
    ...
    Kirigami.PlaceholderMessage {
        ...
        anchors.centerIn: parent
        visible: Merkuro.Filter && Merkuro.Filter.collectionId >= 0 && !Merkuro.CalendarManager.enabledTodoCollections.includes(Merkuro.Filter.collectionId) && parent.count === 0
        text: i18n("Calendar is not enabled")
        helpfulAction: Kirigami.Action {
            icon.name: "gtk-yes"
            text: i18n("Enable")
            onTriggered: Merkuro.CalendarManager.toggleCollection(root.filterCollectionDetails.id)
        }
    }
    ...
    Kirigami.PlaceholderMessage {
        ...
        text: root.showCompleted === Merkuro.TodoSortFilterProxyModel.ShowCompleteOnly ?
            i18n("No tasks completed") : i18n("No tasks left to complete")
        helpfulAction: Kirigami.Action {
            text: i18n("Create")
            icon.name: "list-add"
            onTriggered: CalendarUiUtils.setUpAdd(Merkuro.IncidenceWrapper.TypeTodo, new Date(), Merkuro.Filter.collectionId);
        }
    }

    sourceModel: Merkuro.TodoSortFilterProxyModel {
        id: todoModel
        calendar: Merkuro.CalendarManager.calendar
        incidenceChanger: Merkuro.CalendarManager.incidenceChanger
        filterObject: Merkuro.Filter
        showCompleted: root.showCompleted
        sortBy: root.sortBy
        sortAscending: root.ascendingOrder
        showCompletedSubtodosInIncomplete: Merkuro.Config.showCompletedSubtodos
