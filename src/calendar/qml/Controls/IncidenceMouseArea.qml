...
MouseArea {
    ...
    onClicked: {
        if (mouse.button == Qt.LeftButton) {
            collectionDetails = Merkuro.CalendarManager.getCollectionDetails(mouseArea.collectionId)
    ...
    Component {
        ...
        QQC2.Menu {
            ...
            x: mouseArea.clickX
            z: 1000
            Component.onCompleted: if(mouseArea.collectionId && !mouseArea.collectionDetails) mouseArea.collectionDetails = Merkuro.CalendarManager.getCollectionDetails(mouseArea.collectionId)
            ...
            QQC2.MenuItem {
                ...
                visible: incidenceData.incidenceType === Merkuro.IncidenceWrapper.TypeTodo
            }
            QQC2.MenuItem {
                icon.name: "list-add"
                ...
                enabled: !mouseArea.collectionDetails["readOnly"]
                onClicked: {
                    const parentWrapper = Merkuro.CalendarManager.createIncidenceWrapper()
                    parentWrapper.incidenceItem = Merkuro.CalendarManager.incidenceItem(mouseArea.incidenceData.incidencePtr);
            ...
            QQC2.Menu {
                ...
                enabled: incidenceData.incidenceType === Merkuro.IncidenceWrapper.TypeTodo
                z: 1001

                function setPriority(level) {
                    const wrapper = Merkuro.CalendarManager.createIncidenceWrapper()
                    wrapper.incidenceItem = Merkuro.CalendarManager.incidenceItem(mouseArea.incidenceData.incidencePtr);
                    wrapper.priority = level;
                    Merkuro.CalendarManager.editIncidence(wrapper);
                }
