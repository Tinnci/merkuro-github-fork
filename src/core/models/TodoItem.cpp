...
QString TodoItem::toICalString() const
{
    ...
    switch (status) {
        case Status::NeedsAction:
            statusStr = QLatin1String("NEEDS-ACTION");
            break;
        case Status::InProcess:
            statusStr = QLatin1String("IN-PROCESS");
            break;
        case Status::Completed:
            statusStr = QLatin1String("COMPLETED");
            break;
        case Status::Cancelled:
            statusStr = QLatin1String("CANCELLED");
            break;
    }
...
{
    return uid == other.uid && title == other.title && status == other.status &&
