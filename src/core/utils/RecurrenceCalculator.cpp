...
QList<QDateTime> RecurrenceCalculator::calculateInstances(
    ...
    const QDate& rangeEnd)
{
    ...
    if (event.recurrence.pattern == Recurrence::Pattern::None) {
        if (event.startDateTime.date() >= rangeStart && 
            event.startDateTime.date() <= rangeEnd) {
...
bool RecurrenceCalculator::occurredOn(const CalendarEvent& event, const QDate& date)
{
    ...
    // 单一事件，检查是否在该日期
    if (event.recurrence.pattern == Recurrence::Pattern::None) {
        return event.startDateTime.date() <= date && 
               event.endDateTime.date() >= date;
    }
...
QDate RecurrenceCalculator::getNextOccurrence(
    const CalendarEvent& event,
    const QDate& afterDate)
{
    ...
    // 查询之后 365 天内的实例
    auto instances = calculateInstances(
        event,
        afterDate.addDays(1),
        afterDate.addDays(365)
    );
...
QDate RecurrenceCalculator::getPreviousOccurrence(
    const CalendarEvent& event,
    const QDate& beforeDate)
{
...
QList<QDateTime> RecurrenceCalculator::expandDaily(
    const CalendarEvent& event,
    const QDate& rangeStart,
    const QDate& rangeEnd)
{
    ...
    while (current <= rangeEnd) {
        // 检查递归截止日期
        if (event.recurrence.endDate.isValid() && 
            current > event.recurrence.endDate) {
            break;
        }
...
QList<QDateTime> RecurrenceCalculator::expandWeekly(
    const CalendarEvent& event,
    const QDate& rangeStart,
    const QDate& rangeEnd)
{
...
QList<QDateTime> RecurrenceCalculator::expandMonthly(
    const CalendarEvent& event,
    const QDate& rangeStart,
    const QDate& rangeEnd)
{
...
QList<QDateTime> RecurrenceCalculator::expandYearly(
    const CalendarEvent& event,
    const QDate& rangeStart,
    const QDate& rangeEnd)
{
...
bool RecurrenceCalculator::isException(
    const CalendarEvent& event,
    const QDate& date)
{
